...
plotEmbed <- function(embed, clustering, title = NULL, theWeights = NULL, navigation = TRUE) {
  ...
  while (par != 0) {
    ...
    if (heavyOnly) {
      cnd = theWeights > 0;
      Subtitle = "with nonzero weights only";
    }

    d2 = par %% 10;
    d1 = (par - d2) / 10;
    if (isPDF)
      pdf(paste0("../Results/figs/", experiment$name, "_", experiment$figex, "_", par, ".pdf"))
    ...
    if (navigation) {
      parC = readline(paste("look at plot (0 stop, -1 save, -2 toggle weights)", title, "and \nswitch dimensions up to ", dim(embed)[2], " e.g. 12 >>> "))
    } else {
      parC = "0"
    }

    if (nchar(parC) > 2) {
      cat("bad parameters", parC, "! ")
    } else if (parC == "-1") {
      isPDF = TRUE
    } else if (parC == "-2") {
      heavyOnly = !heavyOnly
    } else if (nchar(parC) < 1 || parC == "0") {
      parC = "0";
      par = 0;
    } else {
        if (nchar(parC) != 2
          ...
          d2 = par %% 10;
          d1 = (par - d2) / 10;
          if (d1 < 1 || d1 > dim(embed)[2] || d2 < 1 || d2 > dim(embed)[2]) {
            par = par0;
            cat("bad parameters", parC, "! ");
          }
        }
