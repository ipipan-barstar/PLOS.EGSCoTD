...
combNSTART = 1000
#---------------- FUNCTIONS --------------
latexMatrix <- function(theMatrix, theComment, theCaption, theLabel, theWidth = 160
                        , cornertext = "TRUE/PRED")
{
  ...
  while (fm < dim(theMatrix0)[2])
  {
    ...
    if (!is.na(fm)) fm = fm - 1 else fm = length(cn);
    ...
    cat("\n\\hline", cornertext);
    for (k in 1:dim(theMatrix)[2]) cat("&", EN.make.tex.names(colnames(theMatrix)[k]));
    cat("\\\\\n");
...
{ #nindex ranging from 0 to 1; ev ordered decreasingly 
  ...
  if (li < 1) li = 1;
  if (ui < 1) ui = 1;
  if (ui == 1) return(ev[ui]);
  if (ui == li) return(ev[ui]);
  return(
    ev[li] + (ev[ui] - ev[li]) * (nidx_inv(nindex, ev) - li)
  );
...
{ noCls = length(table(clsTrue))
  ...
  if (length(combclTab) == noCls) en = 0
    ...
    cat("\nperfect match of clusterings\n") else
  {
    ...
    summx = 0;
    for (j in 1:dim(em)[2]) summx = summx + max(em[, j]);
...
{ no = dim(S)[1];
  d = rep(0, no);
  for (j in 1:no) { d[j] = sum(S[j,]) - S[j, j]; if (abs(d[j]) < 1e-20) d[j] = 1; }
  return(diag(d));
...
similaritySVDapproximation <- function(S, r)
{
  ...
  for (j in 1:dim(Sa)[1]) Sa[j, j] = 0
  for (j in 1:dim(Sa)[1])
    for (k in 1:dim(Sa)[1]) if (Sa[j, k] < 0) { Sa[j, k] = 0; Sa[k, j] = 0; }
  d = Sa - S;
  #print(Sa)
  dd = sum(abs(d))
  for (j in 1:dim(Sa)[1])
  ...
  # highest - if true, the eigenvectors of highest eigenvalues are used, otherwise those of the lowest
{
  ...
  for (j in 1:dim(L)[1])
    if (abs(sss$values[j]) < SETZERO) sss$values[j] = 0
  for (j in 1:dim(L)[1])
    for (k in 1:dim(L)[1])
      if (abs(sss$vectors[j, k]) < SETZERO) sss$vectors[j, k] = 0
  #print(sss$values);
...
{ S = SimilarityMatrix;
  ...
  { if (trueNoCl == 2) trueNoCl = 3;
    fit = kmeans(
      sss$vectors[, length(sss$values) - (trueNoCl - 1):0]
      , center = enforced, nstart = combNSTART);
