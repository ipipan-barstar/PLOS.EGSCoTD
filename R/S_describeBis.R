...
ifelse(!dir.exists(file.path(getwd(), "../Results/figs/")),
       dir.create(file.path(getwd(), "../Results/figs")),
       FALSE)
...
for (isPDF in c(FALSE, TRUE)) {
  ...
  for (kk in names(dst_cats)) {
    points(x[as.character(nnn_cats) == kk], y[as.character(nnn_cats) == kk],
           col = paste0("blue", min(c(which(kk == names(table(nnn_cats))), 4))),
           pch = 14 + min(c(which(kk == names(table(nnn_cats))), 4)))
  ...
  if (!isPDF)
    readline("see ?node connection weights>>");
  if (isPDF)
    dev.off()
}
...
for (isPDF in c(FALSE, TRUE)) {
  if (isPDF)
    pdf(paste0("../Results/figs/", experiment$name, "_", experiment$figex, ".pdf"))
  plot(sort(aDV),
       main = "average similarity sorted - \nblack total, green inside blue outside",
       ylab = "point average similarity",
       ylim = c(0, max(c(aDV, aDVi, aDVo))));
  points(sort(aDVi), col = "green3");
  points(sort(aDVo), col = "blue3");

  if (!isPDF)
    readline("see ?similarities inside versus outside");
  if (isPDF)
    dev.off()
}
...
for (isPDF in c(FALSE, TRUE)) {
  if (isPDF)
    pdf(paste0("../Results/figs/", experiment$name, "_", experiment$figex, ".pdf"))
  plot(sort(aDVi - aDVo),
       main = "average similarity in/out difference",
       ylab = "point average similarity",
       ylim = c(0, max(c(aDV, aDVi, aDVo))));

  if (!isPDF)
    readline("see ?similarities inside  outside differernce>>>");
  if (isPDF)
    dev.off()
}
...
for (isPDF in c(FALSE, TRUE))
{
  ...
  if (!isPDF)
    readline("see ?top and low similarities>>>");
  if (isPDF)
    dev.off()
}
...
for (isPDF in c(FALSE, TRUE)) {
  ...
  if (!isPDF)
    readline("see ?top low sim difference>>>");
  if (isPDF)
    dev.off()
}
